...
namespace CesiumLanguageWriter
{
	...
	private:
		...
		private:
			...
			static std::vector<std::string> BuildDateTimePatterns(const std::vector<std::string>& patterns, int maximumFractionalSeconds)
			{
				std::vector<std::string> result(patterns.size() * (maximumFractionalSeconds + 1));
				for (const std::string& s : patterns)
				{
					size_t pos = s.find_last_of(".f*");
					if (pos = s.size() - 3)
					{
						std::string newString(s);
						newString.replace(pos, 3, "");
						...
						for (int i = 1; i <= maximumFractionalSeconds; i++)
						{
							...
							size_t findPos = tempStr.find("f*");
							while (findPos != tempStr.npos)
							{
								tempStr.replace(findPos, 2, stringbuilder.str());
								findPos = tempStr.find("f*");
							}
			...
			/// </summary>
			static const std::vector<std::string> s_extraDateTimePatternTemplates
			{
				"M/d/yyyy H:mm:ss.f*",
			...
			static const std::vector<std::string> s_parseTimeFormatsTemplate =
				new[]
			{
				"H:m:s.f*",
				"H:m:s",
			...
			static const std::vector<std::string> s_extraDateTimePatterns = BuildDateTimePatterns(s_extraDateTimePatternTemplates, 17);

			static const std::vector<std::string> s_parseTimeFormats = BuildDateTimePatterns(s_parseTimeFormatsTemplate, 17);
			...
			static const std::vector<std::string> s_parseYearDayMonthFormats =
				new[]
			{
				"yyyy/M/dT",
				"M/yyyy/dT",
			...
			static const std::vector<std::string> s_parseYearMonthDayFormats =
				new[]
			{
				"yyyy/M/dT",
				"M/yyyy/dT",
			...
			static const std::vector<std::string> s_parseDayMonthYearFormats =
				new[]
			{
				"yyyy/M/dT",
				"M/yyyy/dT",
			...
			static const std::vector<std::string> s_parseMonthDayYearFormats =
				new[]
			{
				"yyyy/M/dT",
				"M/yyyy/dT",
			...
			static const std::vector<std::string> s_monthDayShortFormats =
				new[]
			{
				"MMMM/d",
				"d/MMM",
			...
			static const std::vector<std::string> s_dayMonthShortFormats =
				new[]
			{
				"d/MMMM",
				"MMM/yy",
